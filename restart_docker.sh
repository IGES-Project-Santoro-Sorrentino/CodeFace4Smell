...
docker exec -it codeface4smell bash -c "./start-server.sh && codeface -j8 run -c codeface.conf -p conf/llvm-project.conf results/ git-repos/ && ./start-dashboard.sh; exec bash"
