...
docker exec -it codeface4smell bash -c "./start_server.sh && codeface -j8 run -c codeface.conf -p conf/qemu.conf results/ git-repos/ && ./deploy-shiny-nginx.sh && codeface -j8 run -c codeface.conf -p conf/php.conf results/ git-repos/; exec bash"
