...
load.config <- function(global.file, project.file=NULL) {
  ...
  if (conf$tagging != "tag" && conf$tagging != "committer2author" &&
      ...
      conf$tagging != "feature" && conf$tagging != "feature_file") {
...
## specifically the codeface and project configuration files. It then creates
...
config.from.args <- function(positional.args=list(), extra.args=list(),
  ...
  option_list <- c(list(
    ...
    make_option(c("-f", "--logfile"), help="logfile", default=default.codeface.log),
    make_option(c("-c", "--config"), default=default.codeface.conf,
                help="global codeface configuration file [%default]"),
...
config.logging.formatter <- function(record) {
  if (is.null(record$logger) || record$logger == "") {
    ...
    from <- paste("[codeface.R.", record$logger, "]", sep="")
  }
