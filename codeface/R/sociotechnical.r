...
load.code.graph <- function(rangedir) {
  ...
  if (!dir.exists(rangedir)) {
    logerror(paste("Directory does not exist:", rangedir))
    stop(paste("Directory does not exist:", rangedir))
  }
  
  ## Check if file exists before trying to open it
  if (!file.exists(mat.file)) {
    logerror(paste("Adjacency matrix file not found:", mat.file))
    logerror(paste("Looking in directory:", rangedir))
    logerror(paste("Directory contents:", paste(list.files(rangedir), collapse=", ")))
    stop(paste("Adjacency matrix file not found:", mat.file, "\nLooking in directory:", rangedir))
  }
...
load.mailinglist.graph <- function(conf, start.date, end.date) {
  ...
  ## if we do not have email collaboration data, create an empty graph
  loginfo(paste("Querying mailing list data from", start.date, "to", end.date))
  
  ## Check if res is empty or invalid
  if (is.null(res) || length(res) == 0) {
    loginfo("No mailing list data found (res is NULL or empty). Creating an empty mailing list graph.")
    return(graph.empty(0, directed=FALSE)) # Return an empty graph
  }
  
  ## Check if res is a data frame with valid structure
  if (!is.data.frame(res) || nrow(res) == 0 || ncol(res) < 2) {
    loginfo(paste("Invalid mailing list data structure. res class:", class(res), 
                  "nrow:", ifelse(is.data.frame(res), nrow(res), "N/A"),
                  "ncol:", ifelse(is.data.frame(res), ncol(res), "N/A")))
    loginfo("Creating an empty mailing list graph.")
    return(graph.empty(0, directed=FALSE)) # Return an empty graph
  }
  
  ## Generate the graph and assign db ids and devs names to vertices
  mail.graph <- tryCatch({
    graph.data.frame(res, directed=FALSE)
  }, error = function(e) {
    logerror(paste("Error creating graph from edgelist:", e$message))
    logerror(paste("res structure:", paste(capture.output(str(res)), collapse="\n")))
    return(graph.empty(0, directed=FALSE))
  })
  
  ## Check if graph has vertices before assigning attributes
  if (vcount(mail.graph) == 0) {
    loginfo("Graph has no vertices. Returning empty graph.")
    return(graph.empty(0, directed=FALSE))
  }
  
  ## Check if vertices have names attribute
  if (is.null(V(mail.graph)$name) || length(V(mail.graph)$name) == 0) {
    logerror("Graph vertices do not have 'name' attribute. Cannot proceed.")
    logerror(paste("Graph has", vcount(mail.graph), "vertices but no names."))
    logerror(paste("res columns:", paste(colnames(res), collapse=", ")))
    return(graph.empty(0, directed=FALSE))
  }
...
config.script.run({
  ...
  loginfo(paste("resdir:", conf$resdir))
  loginfo(paste("tagging:", conf$tagging))
  loginfo(paste("codedir:", codedir))
  loginfo(paste("sociotechdir:", sociotechdir))
  
  ## Check if codedir exists
  if (!dir.exists(codedir)) {
    logerror(paste("Code directory does not exist:", codedir))
    logerror(paste("resdir contents:", paste(list.files(conf$resdir), collapse=", ")))
    stop(paste("Code directory does not exist:", codedir, "\nresdir:", conf$resdir))
  }
