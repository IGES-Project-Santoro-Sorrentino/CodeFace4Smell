      ...
      serialize_params:
        function ($w, wgd) {
          return {
            ...
            id: $w.children(".data-qaid").attr("data-qaid"),
            cls: $w.children(".data-qaclass").attr("data-qaclass")
          }
        }
    });
  });
    ...
    function (message) {
      switch (message.msgname) {
        case 'addWidget':
          ...
          // Add container:'body' to render popover outside the card's stacking context
          var popoverOptions = $.extend({}, message.help, { container: 'body' });
          $(".gridster div#" + message.qaid).popover(popoverOptions);

          // Force z-index when popover is shown to ensure it appears above cards
          $(".gridster div#" + message.qaid).on('shown', function () {
            $('.popover:visible').css({ 'z-index': '9999', 'position': 'absolute' });
          });

          $(".icon-remove-sign").click(function () {
            var gridster = $(".gridster > ul").gridster().data('gridster');
            var el = $(this).parent();
            var qaid = $(this).parent().children(".data-qaid").attr("data-qaid");
