...
CREATE UNIQUE INDEX `externalId_UNIQUE` ON `codeface`.`issue` (`externalId` ASC) ;
...
CREATE INDEX `watcher_list_issue_idx` ON `codeface`.`watcher_list` (`issueId` ASC) ;
...
CREATE INDEX `issue_change_issue_idx` ON `codeface`.`issue_change` (`issueId` ASC) ;

CREATE INDEX `issue_change_who_idx` ON `codeface`.`issue_change` (`who` ASC) ;
...
CREATE TABLE IF NOT EXISTS `codeface`.`mail` (
  ...
  `subject` VARCHAR(255) NULL DEFAULT NULL,
  `creationDate` DATETIME NOT NULL,
  `creationDateOffset` INT(11) NOT NULL,
...
CREATE INDEX `field_change_issue_change_idx` ON `codeface`.`field_change` (`issueChangeId` ASC) ;
...
CREATE INDEX `issue_commit_issue_idx` ON `codeface`.`issue_commit` (`issueId` ASC) ;

CREATE INDEX `issue_commit_commit_idx` ON `codeface`.`issue_commit` (`commitId` ASC) ;
