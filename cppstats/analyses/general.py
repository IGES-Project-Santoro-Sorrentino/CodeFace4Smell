...
def __getNumOfFilesPerFeatureStats(filetofeatureconstants):
    ...
    numbers = [len(v) for v in list(featureconstantstofiles.values())]
...
#                            " match (%s, %s)!" % (str(tmpdepth), str(depth)))
    ...
    astats[__statsorder.HET.index] = len(list(het.keys()))
    astats[__statsorder.HOM.index] = len(list(hom.keys()))
    astats[__statsorder.HOHE.index] = len(list(hethom.keys()))
