...
def _getFeatureStats(features):
    ...
    tmp = [item for (_, item) in features.values()]
    tmp = _flatten(tmp)
    floflist = [n.count('\n') for n in tmp]
...
def _getFeaturesDepthOne(features):
    ...
    nof1 = [sig_depth_code for sig_depth_code in iter(features.items()) if sig_depth_code[1][0] == 1]
    return nof1
...
def apply(folder, options):
    ...
    annotations = [a_flag_b_c[1][0] for a_flag_b_c in afeatureitems]
    annotations3andmore = [a for a in annotations if len(a) > 2]
    annotations3andmore = uniqueItems(annotations3andmore)
    annotations3andmore = [set(s) for s in annotations3andmore]
    ...
    for annotation in annotations3andmore:
        combinations = [set(s) for s in list(itertools.combinations(annotation, 2))]
